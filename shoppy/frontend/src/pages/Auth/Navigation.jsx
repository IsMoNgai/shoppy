...
const Navigation = () => {
  ...
  } //working with database so need async

  return (
    <div style={{ zIndex : 999 }} className={`${showSlidebar ? "hidden" : "flex"} xl:flex lg:flex md:hidden sm:hidden flex-col justify-between p-4 text-white bg-black w-[4%] hover:w-[15%] h-[100vh] fixed`} id="navigation-container">
      ...
      <div className="relative">
        <button onClick={toggleDropdown} className="flex item-center text-grey-8000 focus:outline-none">
          ...
          {dropDownOpen && userInfo && (
            <ul className={`absolute right-0 mt-2 mr-14 space-y-2 bg-gray-900 text-white ${!userInfo.isAdmin ? '-top-20' : '-top-80'}`}>
              {userInfo.isAdmin && (
                <>
                  <li>
                ...
                <li>
                  <Link to="/admin/logout" onClick={logoutHandler} className="block px-4 py-2 hover:bg-gray-700">
                    Logout
                  </Link>
                </li>
            </ul>
